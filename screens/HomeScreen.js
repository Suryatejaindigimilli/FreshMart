...
//             >
//               <View>
//                 <Icon name="cart-outline" size={22} color="#1B1B1B" />
//                 {getCartCount() > 0 && (
...
//   categoryId: item.id,     // ex: "vegetables"
//   title: item.title        // ex: "Vegetables"
// })}
//                 activeOpacity={0.85}
//               >
...
const K_LOC = 'current_location_label';
...
const askMic = async () => {
  ...
  const r = await PermissionsAndroid.request(
    PermissionsAndroid.PERMISSIONS.RECORD_AUDIO,
    {
      ...
      message: 'Allow microphone access for voice search.',
...
const HomeScreen = ({ route, navigation }) => {
  const scrollRef = useRef(null); // banner list ref
  const bnIdxRef = useRef(0);
  ...
  const ignoreErrRef = useRef(false);
  ...
  const [nm, setNm] = useState('User');
  const [gr, setGr] = useState(getGreeting());

  const [loc, setLoc] = useState(route?.params?.location || 'KBPB');
    ...
    if (Platform.OS === 'android') {
      ...
      const r = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION,
        {
          ...
          message: 'Allow location to show delivery area near you.',
          buttonPositive: 'Allow',
          buttonNegative: 'Deny',
        }
      );
  ...
  const loadSavedLocation = useCallback(async () => {
    try {
      const s = await AsyncStorage.getItem(K_LOC);
      if (s) setLoc(s);
    } catch (e) {}
  }, []);
    ...
    Geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          setLoc(label);
          await AsyncStorage.setItem(K_LOC, label);
        } catch (e) {}
      },
      () => {},
      {
  ...
  useFocusEffect(
    ...
    }, [fetchCurrentLocation, loadSavedLocation])
  );
  ...
  const hydrateFromCache = async () => {
    const [c, s] = await Promise.all([
      readCache(K_CAT, K_CAT_TS),
      readCache(K_SHOP, K_SHOP_TS),
    ]);
    if (c) {
      setCategories(c);
      setLoadingCategories(false);
  ...
  // ✅ BANNERS: smooth auto-scroll with FlatList + pause when user drags
  useEffect(() => {
    const id = setInterval(() => {
      if (dragRef.current) return;

      const n = (bnIdxRef.current + 1) % posters.length;
      scrollRef.current?.scrollToIndex({ index: n, animated: true });
      bnIdxRef.current = n;
      setPosterIndex(n);
    }, 3000);
  ...
  useEffect(() => {
    ...
    Voice.onSpeechError = (e) => {
      ...
      const m = String(e?.error?.message ?? '').toLowerCase();

      if (c === '7' || m.includes('no match')) return;

      Alert.alert('Voice error', e?.error?.message || 'Try again');
    };

    Voice.onSpeechResults = (e) => {
      const txt = e?.value?.[0] || '';
      if (txt) setSearchText(txt);
    };

    return () => {
      ignoreErrRef.current = true;
  ...
  const stopVoice = async () => {
    try {
      ...
      await Voice.stop();
      setListening(false);
      setTimeout(() => (ignoreErrRef.current = false), 300);
    } catch {
      setListening(false);
    }
  };
      ...
      >
        <View style={styles.topWrap}>
          ...
          <View style={styles.searchWrapper}>
            ...
            <TextInput
              ...
              style={styles.searchInput}
            />
            <TouchableOpacity style={styles.micBtn} activeOpacity={0.8} onPress={onMicPress}>
              <Icon
          ...
          {listening && (
            <Text style={{ marginTop: 8, color: '#18A957', fontWeight: '700', fontSize: 12 }}>
              Listening… speak now
            </Text>
          )}
        </View>
        ...
        <View style={styles.carouselWrap}>
          <FlatList
            ...
            onScrollBeginDrag={() => (dragRef.current = true)}
            onScrollEndDrag={() => (dragRef.current = false)}
            onMomentumScrollEnd={onBannerEnd}
          ...
          <View style={styles.dotsRow}>
            {posters.map((_, i) => (
              <View
                key={i}
                style={[styles.dot, posterIndex === i ? styles.dotActive : styles.dotIdle]}
              />
            ))}
          </View>
        </View>
        ...
        <View style={styles.sectionHead}>
          <Text style={styles.sectionTitle}>Categories</Text>
          <TouchableOpacity activeOpacity={0.8} onPress={() => setShowAllCats(v => !v)}>
            <Text style={styles.seeAll}>{showAllCats ? 'See less' : 'See all'}</Text>
          </TouchableOpacity>
        </View>
        ...
        {loadingCategories ? (
          ...
          <View style={styles.gridContainer}>
                {(showAllCats ? catsShown : catsShown.slice(0, 6)).map((item, idx) => (
                  <TouchableOpacity
                    key={String(item?.id ?? item?._id ?? item?.cat_id ?? idx)}
                    style={styles.catCard}
                    onPress={() => navigation.navigate('CategoryScreen', { category: item })}
                    activeOpacity={0.85}
                  >
