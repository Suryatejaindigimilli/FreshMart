...
const HomeScreen = ({ route, navigation }) => {
  ...
  const [loc, setLoc] = useState(route?.params?.location || 'KPHB');
  const [locOpen, setLocOpen] = useState(false);
  const [locPick, setLocPick] = useState(route?.params?.location || 'KPHB');
  ...
  const applyLoc = async () => {
    try {
      setLoc(locPick);
      await AsyncStorage.setItem(K_LOC, locPick);
    } catch (e) {}
    setLocOpen(false);
  };
  ...
  useEffect(() => {
    ...
    const unsub = navigation.addListener('focus', loadLoc);
    return () => unsub && unsub();
  }, []);
        ...
        <View style={styles.topWrap}>
          <View style={styles.headerRow}>
            <View>
              ...
              <TouchableOpacity style={styles.locationRow} activeOpacity={0.85} onPress={openLocPicker}>
                <Icon name="location-outline" size={16} color="#18A957" />
                <Text style={styles.locationText}>{loc}</Text>
                <Icon name="chevron-down-outline" size={14} color="#3A3A3A" style={{ marginLeft: 4 }} />
              </TouchableOpacity>
            </View>
          ...
          <TouchableOpacity style={styles.searchWrapper} activeOpacity={1}>
            ...
            />
            <TouchableOpacity style={styles.micBtn} activeOpacity={0.8} onPress={onMic}>
              <Icon
      ...
      <Modal visible={locOpen} transparent animationType="slide" onRequestClose={() => setLocOpen(false)}>
        <View style={styles.mOverlay}>
          <View style={styles.mBox}>
            <Text style={styles.mTitle}>Choose delivery location</Text>

            <View style={styles.mPickerWrap}>
              <Picker selectedValue={locPick} onValueChange={setLocPick} style={styles.mPicker}>
            ...
            <View style={styles.mRow}>
              <TouchableOpacity style={styles.mBtn2} activeOpacity={0.85} onPress={() => setLocOpen(false)}>
                <Text style={styles.mBtn2Text}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.mBtn1} activeOpacity={0.85} onPress={applyLoc}>
                <Text style={styles.mBtn1Text}>Apply</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  mTitle: { fontSize: 16, fontWeight: '900', marginBottom: 10, color: '#111' },
  mPickerWrap: { borderWidth: 1, borderColor: '#eee', borderRadius: 12, overflow: 'hidden' },
  mPicker: { color: '#000' },
  mRow: { flexDirection: 'row', gap: 10, marginTop: 12 },
  mBtn1: { flex: 1, backgroundColor: '#18A957', paddingVertical: 12, borderRadius: 12 },
  mBtn1Text: { color: '#fff', fontWeight: '900', textAlign: 'center' },
  mBtn2: { flex: 1, borderWidth: 1, borderColor: '#ddd', paddingVertical: 12, borderRadius: 12 },
  mBtn2Text: { color: '#111', fontWeight: '900', textAlign: 'center' },
});
